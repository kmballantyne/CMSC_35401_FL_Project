...
# Keep only digital and original images
subset_df = train_df[train_df['TransformCategory'].isin(['digital', 'original'])]

# Sample 2500 from each
sampled_digital = subset_df[subset_df['TransformCategory'] == 'digital'].sample(n=2500, random_state=42)
sampled_original = subset_df[subset_df['TransformCategory'] == 'original'].sample(n=2500, random_state=42)
...
# Original → clients 2 and 3
clients[2] = sampled_original.iloc[:1000].copy()
clients[2]['client_id'] = 2

clients[3] = sampled_original.iloc[1000:2000].copy()
clients[3]['client_id'] = 3

# Combine leftovers into a fifth client (could also be spatial or random mix)
remaining = pd.concat([
    sampled_digital.iloc[2000:], 
    sampled_original.iloc[2000:]
